...
%load data
data1 = importdata("mcf_stats.txt");
n= length(data1.data);
...
b.FaceColor = 'flat';
b.CData(iEdap,:) = [1 0 0];
title("EDAP of every simulation model (mcf)");
xlabel("model");
ylabel("EDAP");
...
%area-associativity
areaValues= [area(2) area(3) area(7)];
figure(2)
...
title("mcf: Area vs Cache Line size");
ylabel("Area (mm^2)");
...
%power-associativity
powerValues= [power(2) power(3) power(7)];
figure(6)
...
xticks([1 2 3]);
xticklabels({'(1,1,4)','(2,2,8)','(4,4,16)'});
title("mcf: Power vs Associativity");
ylabel("Power (W)");
xlabel("Associativity (L1D,L1I,L2)");
text(2.5,20,{"L1D-size=64KB","L1I-size=32KB","L2-size=2MB","CL-size=64"});

%power-l1 size
powerValues= [power(1) power(3) power(8)];
figure(7)
plot(1:3,powerValues,'Marker','o');
ylim([1 30]);
xlim([0.9 3.1]);
...
figure(10);
scatter(z_f,z_edap);
title("F(Standardized) vs EDAP(Standardized) - mcf");
xlabel("F");
ylabel("EDAP");
