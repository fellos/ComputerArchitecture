%hmmer graphs for lab3

%load data
data1 = importdata("hmmer_stats.txt");
n= length(data1.data);
...
b.FaceColor = 'flat';
b.CData(iEdap,:) = [1 0 0];
title("EDAP of every simulation model (hmmer)");
xlabel("model");
ylabel("EDAP");
...
%area-associativity
areaValues= [area(2) area(4) area(7)];
figure(2)
...
title("hmmer: Area vs Cache Line size");
ylabel("Area (mm^2)");
...
%power-associativity
powerValues= [power(2) power(4) power(7)];
figure(6)
...
xticks([1 2 3]);
xticklabels({'(1,1,4)','(2,2,8)','(4,4,16)'});
title("hmmer: Power vs Associativity");
ylabel("Power (W)");
xlabel("Associativity (L1D,L1I,L2)");
text(2.5,20,{"L1D-size=64KB","L1I-size=32KB","L2-size=2MB","CL-size=64"});

%power-l1 size
powerValues= [power(1) power(4) power(8)];
figure(7)
plot(1:3,powerValues,'Marker','o');
ylim([1 30]);
xlim([0.9 3.1]);
...
figure(10);
scatter(z_f,z_edap);
title("F(Standardized) vs EDAP(Standardized) - hmmer");
xlabel("F");
ylabel("EDAP");
text(1.5,3,sprintf("r = %f",corr2(f,edap)),'FontSize',12,'Color','blue');