%libm graphs for lab3

%load data
data1 = importdata("libm_stats.txt");
n= length(data1.data);
...
b.FaceColor = 'flat';
b.CData(iEdap,:) = [1 0 0];
title("EDAP of every simulation model (libm)");
xlabel("model");
ylabel("EDAP");
...
title("libm: Area vs Associativity");
ylabel("Area (mm^2)");
...
title("libm: Area vs Cache Line size");
ylabel("Area (mm^2)");
...
xticks([1 2 3]);
xticklabels({'(1,1,4)','(2,2,8)','(4,4,16)'});
title("libm: Power vs Associativity");
ylabel("Power (W)");
xlabel("Associativity (L1D,L1I,L2)");
...
figure(10);
scatter(z_f,z_edap);
title("F(Standardized) vs EDAP(Standardized) - libm");
xlabel("F");
ylabel("EDAP");
text(1.5,1,sprintf("r = %f",corr2(f,edap)),'FontSize',12,'Color','blue');